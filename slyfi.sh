...
echo
echo "   ssss   ll    yy   yy      ffffff   ii "
echo "  ss  ss  ll     yy yy       ff          "
echo "   ss     ll       y     -   ffff     ii "
echo "    ssss  ll       y     -   f        ii "
echo "       ss lllll    y         f        ii "
echo "   sss ss                             ii "
echo "    ssss      BY SINISTER_MACHINE     ii "
echo
# Check for wireless adaptor
iwconfig > tmp 2>/dev/null

if [ ! -s tmp ]; then
     zenity --error --text "NO WIFI DETECTED?! If you are using a VM, USB card MUST be enabled."
     rm tmp 2>/dev/null
     exit
fi

echo -e "\e[1;33m[*] Setting sinister variables.\e[0m"
...
if grep -q 'Injection is working like a crackwhore on the corner!' tmp; then
	rm tmp 2>/dev/null
else
